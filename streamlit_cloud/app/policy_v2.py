...
class CandidateFilter:
    ...
    # OR 조건(합집합): base 필터를 적용한 뒤, any_of 각 분기 결과를 합쳐 최종 후보를 만든다.
    # 예) RAM>=3 OR GPU contains "powervr"
    # candidate_filter:
    #   platform: android,ios
    #   any_of:
    #     - min_values: {ram_gb: 3}
    #     - include_contains: {gpu: [powervr]}
    any_of: tuple["CandidateFilter", ...] = ()
    availability: AvailabilityFilter = AvailabilityFilter()


@dataclass(frozen=True)
class DedupeWithinRank:
    # NOTE: Engine-fixed dedupe key (v2): cpu + ram_gb
    # - GPU is intentionally ignored for dedupe to reduce over-fragmentation.
    key: tuple[str, ...] = ("cpu", "ram_gb")
...
def load_policy_v2(path: str | Path) -> PolicyV2:
    ...
    def _parse_availability(d: Any, *, default: AvailabilityFilter) -> AvailabilityFilter:
        if not isinstance(d, dict):
            return default
        return AvailabilityFilter(
            usable_only=bool(d.get("usable_only", default.usable_only)),
            rentable_only=bool(d.get("rentable_only", default.rentable_only)),
        )

    def _parse_candidate_filter(d: Any, *, inherit_platform: str) -> CandidateFilter:
        if not isinstance(d, dict):
            d = {}

        # platform: if missing, inherit from parent so nested any_of doesn't accidentally default to "android"
        platform = str(d.get("platform") or inherit_platform or "android")

        # availability
        av = _parse_availability(d.get("availability"), default=AvailabilityFilter())

        # numeric_ranges (optional)
        nr_obj = d.get("numeric_ranges", {}) or {}
        ...
        iv_obj = d.get("include_values", {}) or {}
        ...
        ic_obj = d.get("include_contains", {}) or {}
        ...
        # any_of (optional)
        any_of_list = d.get("any_of", None)
        any_of: list[CandidateFilter] = []
        if isinstance(any_of_list, list):
            for item in any_of_list:
                if isinstance(item, dict):
                    any_of.append(_parse_candidate_filter(item, inherit_platform=platform))

        return CandidateFilter(
            platform=platform,
            required_fields=tuple(str(x) for x in (d.get("required_fields", []) or [])),
    ...
    cf_obj = _get(obj, "candidate_filter", {}) or {}
    base_platform = str(_get(obj, "candidate_filter.platform", "android") or "android")
    cf = _parse_candidate_filter(cf_obj, inherit_platform=base_platform)

    # dedupe
    dwr = DedupeWithinRank(
        key=tuple(str(x) for x in (_get(obj, "dedupe.within_rank.key", ["cpu", "ram_gb"]) or [])),
