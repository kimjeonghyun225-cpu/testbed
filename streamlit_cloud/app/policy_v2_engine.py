    ...
    *,
    ...
    for rk in RANK_ORDER:
                    ...
                    decision_rows.append(
                        {
                            ...
                            "axes_key": axk,
                            "stage": "core_rep",
                            # show which device was chosen as the representative for this profile
                            "why_short": (
                                "같은 스펙은 1대만 남기기 (선정기기: "
                                + (
                                    (str(r.get("No") or "").strip() or "-")
                                    + " / "
                                    + (
                                        str(r.get("product_name") or r.get("model_name") or "").strip()
                                        or str(r.get("device_id") or "").strip()
                                        or "-"
                                    )
                                )
                                + ")"
        ...
        if len(selected_rows) < target_n:
            ...
            if use_manu:
                def _on_extra_take_brand(rr: pd.Series) -> None:
                    # Keep behavior: product_name global cap accounting
                    _mark_taken(rr)
        ...
        # fallback fill (AUTO-RELAX):
        # If we still cannot reach target_n, relax *duplicate restrictions* to fill the quota.
        # - ignore product_name cap (max_per_product_name) for fallback
        # - allow duplicates within the same profile_core up to target_n
        # - ignore allow_axes constraint (full relaxation)
        if len(selected_rows) < target_n:
            deficit0 = int(target_n - len(selected_rows))
            logs.append(
                f"[fallback_relax] rank={rk} shortage={deficit0} -> relax dedupe/product caps to fill target_n"
            )
            # Re-scan remaining candidates in original order (including those previously blocked by product_name cap).
            sub_sorted3 = sub_sorted.copy()
            if used_ids and "device_id" in sub_sorted3.columns:
                sub_sorted3 = sub_sorted3[~sub_sorted3["device_id"].fillna("").astype(str).isin(used_ids)].copy()

            relax_max_dup = max(int(target_n), int(max_dup))
            for _, rr in sub_sorted3.iterrows():
                if len(selected_rows) >= target_n:
                    break
                did = str(rr.get("device_id") or "")
                if not did or did in used_ids:
                    continue
                core = str(rr.get("_profile_core") or "")
                # fully relax: allow duplicates per core up to relax_max_dup
                if core_counts.get(core, 0) >= relax_max_dup:
                    continue

                # NOTE: do NOT apply product_name global cap in fallback
                selected_rows.append(rr)
                used_ids.add(did)
                core_counts[core] = core_counts.get(core, 0) + 1
                axk2 = str(rr.get("_axes_key") or "")
                ...
                decision_rows.append(
                    {
                        ...
                        "profile_core": core,
                        "axes_key": axk2,
                        "stage": "fallback_relax",
                        "why_short": "부족하면 중복도 포함해서 채우기",
                        "why": "부족분 충족을 위해 중복제한 완화(fallback)로 추가 선정",
                        "why_detail": (
                            f"[fallback_relax] Rank={rk} 목표={target_n}대 대비 부족분({deficit0})을 채우기 위해 중복제한을 완화\n"
                            f"- relax: ignore product_name cap, allow duplicates per profile up to {relax_max_dup}\n"
                            f"- selection_progress: selected={len(selected_rows)}/{target_n}\n"
                            f"- profile_core={core}\n"
                            f"- tie_keys: year={yv} disp_score={dv} gpu_pri={gp}\n"
                        ),
                    }
