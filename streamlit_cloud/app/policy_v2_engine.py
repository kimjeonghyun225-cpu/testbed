...
def _sort_for_pick(df: pd.DataFrame, tie_breakers: tuple[object, ...]) -> pd.DataFrame:
    ...
    """
    # Always enforce no_prefix_priority as the first tie-breaker (default: android AP>AT>R, ios IP>IT>R),
    # even if the YAML does not specify it.
    has_npp = any(isinstance(tb, dict) and "no_prefix_priority" in tb for tb in tie_breakers)
    tb_eff = tie_breakers if has_npp else ({"no_prefix_priority": {"android": ["AP", "AT", "R"], "ios": ["IP", "IT", "R"]}},) + tie_breakers

    tmp = _tie_key(df, tb_eff)
    cols: list[str] = []
    asc: list[bool] = []
...
def run_policy_v2(
    *,
    ...
    # - dedupe key is always (cpu, gpu, ram_gb) regardless of YAML.
    ...
    logs.append("[dedupe_fixed] key=(cpu,gpu,ram_gb) max_per_product_name=1 (GLOBAL across ranks)")
