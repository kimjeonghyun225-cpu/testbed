...
class PolicyV2RunInputs:
    ...
    # 같은 스펙(프로파일) 그룹 내 대표 1개를 고르는 기준을 "회전"시키기 위한 실행 nonce
    # - 0이면 기존과 동일하게 결정론적으로 첫 후보를 선택
    # - 1,2,3...으로 증가시키면 동일 조건 재실행 시 같은 스펙에서 다른 기기가 선택될 수 있음
    rotation_nonce: int = 0


def _stable_int(s: str) -> int:
    """
    Python의 hash()는 프로세스마다 시드가 달라질 수 있어(stable 아님),
    md5 기반으로 안정적인 정수 해시를 만든다.
    """
...
def _brand_diverse_fill(
    ...
    *,
    ...
    while remain and len(selected_rows) < target_n:
        # NOTE: remain is a list[pd.Series]. Do NOT use list.remove(series) because it triggers
        # pandas Series equality semantics (== returns Series) -> ValueError: truth value of a Series is ambiguous.
        best_pos = min(range(len(remain)), key=lambda i: _score_row(remain[i]))
        best_rr = remain[best_pos]
...
def _tie_key(df: pd.DataFrame, tie_breakers: tuple[object, ...]) -> pd.DataFrame:
    ...
    for tb in tie_breakers:
        # NOTE: We accept any dict containing "no_prefix_priority".
        # If config value is missing/invalid, fall back to default order.
        if isinstance(tb, dict) and "no_prefix_priority" in tb:
            ...
            # iOS prefix priority (ENGINE-FIXED default): IP > IR > IT > R
            i_list = [str(x).strip().upper() for x in (cfg.get("ios") or ["IP", "IR", "IT", "R"]) if str(x).strip()]
...
def _sort_for_pick(df: pd.DataFrame, tie_breakers: tuple[object, ...]) -> pd.DataFrame:
    ...
    """
    # ENGINE-FIXED DEFAULT tie-breakers:
    # - no_prefix_priority (always first)
    # - newest_release_year
    # - higher_display_resolution (display_w * display_h)
    # These are enforced regardless of YAML so results are consistent across UI/prompt/manual YAML.
    default_npp: dict[str, object] = {
        # 요청: AOS는 AP(=A) > AT > R, iOS는 IP > IR > R 우선.
        # (iOS에서 IT도 존재할 수 있어 IR 다음으로 둔다.)
        "no_prefix_priority": {"android": ["AP", "AT", "R"], "ios": ["IP", "IR", "IT", "R"]}
    }
    default_tail: tuple[object, ...] = ("newest_release_year", "higher_display_resolution")

    # Ensure no_prefix_priority exists first
    has_npp = any(isinstance(tb, dict) and "no_prefix_priority" in tb for tb in tie_breakers)
    tb_eff_list: list[object] = list(tie_breakers)
    if not has_npp:
        tb_eff_list = [default_npp, *tb_eff_list]

    # Ensure default string tie-breakers exist (append if missing)
    for d in default_tail:
        if d not in tb_eff_list:
            tb_eff_list.append(d)

    # De-dup while preserving order (important if YAML already included the defaults)
    seen: set[str] = set()
    tb_eff: list[object] = []
    for tb in tb_eff_list:
        k = str(tb)
        if k in seen:
            continue
        seen.add(k)
    ...
    tmp = _tie_key(df, tuple(tb_eff))
    cols: list[str] = []
    asc: list[bool] = []
    # apply known tie breakers in order (no_prefix_priority is always first)
    cols.append("_no_pri"); asc.append(True)
    for tb in tb_eff:
        if isinstance(tb, dict) and "no_prefix_priority" in tb:
            continue
        if tb == "newest_release_year":
...
def _apply_candidate_filter(df: pd.DataFrame, policy: PolicyV2, logs: list[str]) -> pd.DataFrame:
    from .policy_v2 import CandidateFilter

    def _apply_and_filters(df_in: pd.DataFrame, cf: CandidateFilter, label: str) -> pd.DataFrame:
        out = df_in.copy()

        def _lg(msg: str) -> None:
            logs.append(f"[{label}] {msg}")

        # platform filter (android/ios/both)
        plat_raw = str(getattr(cf, "platform", "") or "").strip().lower()
        if plat_raw:
            ...
            if plats and plats != {"android", "ios"}:
                if "platform" in out.columns:
                    out["_platform_norm"] = (
                        out["platform"].fillna("").astype(str).str.strip().str.lower().replace({"aos": "android"})
                    )
                    before = int(len(out))
                    out = out[out["_platform_norm"].isin(plats)].copy()
                    after = int(len(out))
                    _lg(f"platform_filter platforms={sorted(plats)} kept={after} dropped={before-after}")
                    out = out.drop(columns=["_platform_norm"], errors="ignore")
                else:
                    _lg("warn platform_filter: platform set but platform column missing; skip")

        # availability (ENGINE-FIXED):
        # - Always keep only usable devices (available=True) regardless of policy YAML.
        # - rentable_only is intentionally not supported/used.
        if "available" in out.columns:
            before = int(len(out))
            out = out[out["available"].fillna(False).astype(bool)].copy()
            after = int(len(out))
            _lg(f"availability_fixed available=true kept={after} dropped={before-after}")
        else:
            _lg("warn availability_fixed: available column missing; skip")

        # required fields
        req = set(cf.required_fields or ())
        missing = [f for f in req if f not in out.columns]
        if missing:
            _lg(f"warn required_fields missing columns={missing} (will reduce candidates)")
        for f in req:
        ...
        nr = getattr(cf, "numeric_ranges", {}) or {}
        if isinstance(nr, dict) and nr:
            for col, rng in nr.items():
                if col not in out.columns:
                    _lg(f"warn numeric_ranges: missing column={col}; skip")
                    continue
                try:
                    lo, hi = float(rng[0]), float(rng[1])
                except Exception:
                    _lg(f"warn numeric_ranges: invalid range for {col}: {rng}; skip")
                    continue
                before = int(len(out))
                ...
                out = out[keep].copy()
                after = int(len(out))
                _lg(f"numeric_ranges {col} in [{lo}, {hi}] kept={after} dropped={before-after}")

        # include_values (case-insensitive exact match)
        iv = getattr(cf, "include_values", {}) or {}
        if isinstance(iv, dict) and iv:
            for col, vals in iv.items():
                ...
                if col == "brand" and "manufacturer" in out.columns:
                    col = "manufacturer"
                if col not in out.columns:
                    _lg(f"warn include_values: missing column={col}; skip")
                    continue
                ...
                after = int(len(out))
                _lg(
                    f"include_values {col} in {sorted(want)[:10]}{'...' if len(want) > 10 else ''} kept={after} dropped={before-after}"
                )

        # include_contains (case-insensitive substring match)
        ic = getattr(cf, "include_contains", {}) or {}
        if isinstance(ic, dict) and ic:
            for col, vals in ic.items():
                ...
                if col == "brand" and "manufacturer" in out.columns:
                    col = "manufacturer"
                if col not in out.columns:
                    _lg(f"warn include_contains: missing column={col}; skip")
                    continue
        ...
        if isinstance(mv, dict) and mv:
            for col, v in mv.items():
                ...
                if not col:
                    continue
                if col not in out.columns:
                    _lg(f"warn min_values: missing column={col}; skip")
                    continue
                try:
                    th = float(v)
                except Exception:
                    _lg(f"warn min_values: invalid value for {col}: {v}; skip")
                    continue
                before = int(len(out))
                s = pd.to_numeric(out[col], errors="coerce")
                keep = s.notna() & (s >= th)
                out = out[keep].copy()
                after = int(len(out))
                _lg(f"min_values {col} >= {th} kept={after} dropped={before-after}")

        return out

    def _eval(df_in: pd.DataFrame, cf: CandidateFilter, label: str) -> pd.DataFrame:
        # 1) base AND filters
        base = _apply_and_filters(
            df_in,
            CandidateFilter(
                platform=cf.platform,
                required_fields=cf.required_fields,
                include_values=cf.include_values,
                include_contains=cf.include_contains,
                min_values=cf.min_values,
                numeric_ranges=cf.numeric_ranges,
                any_of=(),
                availability=cf.availability,
            ),
            label=label,
        )

        # 2) OR union of branches (if any)
        branches = tuple(cf.any_of or ())
        if not branches:
            return base
        ...
        for i, sub in enumerate(branches):
            try:
                parts.append(_eval(base, sub, label=f"{label}.any_of[{i}]"))
            except Exception as e:
                logs.append(f"[{label}] warn any_of[{i}] failed: {e}")
                continue

        if not parts:
            logs.append(f"[{label}] warn any_of specified but no valid branches; returning base (AND only)")
            return base

        merged = pd.concat(parts, axis=0)
        # de-duplicate by index to produce a clean union
        merged = merged.loc[~merged.index.duplicated(keep="first")]
        logs.append(f"[{label}] any_of union branches={len(parts)} base_kept={int(len(base))} union_kept={int(len(merged))}")
        return merged

    return _eval(df, policy.candidate_filter, label="candidate_filter")
...
def run_policy_v2(
    *,
    ...
    # rotation nonce (per run) for picking a different representative within the same spec/profile_core
    rotation_nonce = int(getattr(inputs, "rotation_nonce", 0) or 0)

    # required devices by No: force include (resolve from df0 BEFORE candidate_filter)
    required_nos = [str(x).strip() for x in inputs.required_nos if str(x).strip()]
    required_nos_set = {str(x).strip().upper() for x in required_nos if str(x).strip()}
    required_ids: set[str] = set()
    if required_nos and "No" in df0.columns:
        tmp = df0[["No", "device_id"]].copy() if "device_id" in df0.columns else df0[["No"]].copy()
        tmp["No"] = tmp["No"].fillna("").astype(str).str.strip().str.upper()
        if "device_id" in tmp.columns:
        ...
        for n in required_nos:
            did = no_to_id.get(str(n).strip().upper(), "")
            if did:
                required_ids.add(did)
    elif required_nos and "device_id" in df0.columns:
        required_ids |= set(required_nos)

    if required_nos and (not required_ids) and "No" in df0.columns and "device_id" in df0.columns:
        logs.append(f"[warn] required_nos specified but no device_id resolved: {required_nos[:10]}{'...' if len(required_nos) > 10 else ''}")

    # candidate filter
    df = _apply_candidate_filter(df0, policy, logs)
    stats["after_candidate_filter_rows"] = int(len(df))

    # Force include required rows even if candidate_filter would drop them.
    if required_nos or required_ids:
        req_rows = df0.head(0).copy()
        if required_ids and "device_id" in df0.columns:
            req_rows = df0[df0["device_id"].fillna("").astype(str).str.strip().isin(required_ids)].copy()
        elif required_nos_set and "No" in df0.columns:
            req_rows = df0[df0["No"].fillna("").astype(str).str.strip().str.upper().isin(required_nos_set)].copy()

        if not req_rows.empty:
            before = int(len(df))
            df = pd.concat([df, req_rows], axis=0)
            df = df.loc[~df.index.duplicated(keep="first")]
            added = int(len(df) - before)
            if added > 0:
                logs.append(f"[required_force_include] added={added} (may violate candidate_filter)")
    ...
    # exclude previous version by No/device_id (ABSOLUTE EXCLUDE)
    exclude_prev_nos = [str(x).strip() for x in (inputs.exclude_prev_version_nos or []) if str(x).strip()]
    exclude_prev_nos_set = {str(x).strip().upper() for x in exclude_prev_nos if str(x).strip()}
    exclude_prev_ids: set[str] = set()
    if exclude_prev_nos_set and "No" in df0.columns and "device_id" in df0.columns:
        tmp2 = df0[["No", "device_id"]].copy()
        tmp2["No"] = tmp2["No"].fillna("").astype(str).str.strip().str.upper()
        tmp2["device_id"] = tmp2["device_id"].fillna("").astype(str).str.strip()
        no_to_id2 = {str(r["No"]): str(r.get("device_id") or "") for _, r in tmp2.iterrows() if str(r["No"]).strip()}
        for n in exclude_prev_nos_set:
            did = no_to_id2.get(n, "")
            if did:
                exclude_prev_ids.add(did)

    if exclude_prev_nos_set or exclude_prev_ids:
        stats["prev_version_exclude_enabled"] = True
        drop_abs = pd.Series([False] * len(df), index=df.index)
        if exclude_prev_ids and "device_id" in df.columns:
            drop_abs = drop_abs | df["device_id"].fillna("").astype(str).str.strip().isin(exclude_prev_ids)
        if exclude_prev_nos_set and "No" in df.columns:
            drop_abs = drop_abs | df["No"].fillna("").astype(str).str.strip().str.upper().isin(exclude_prev_nos_set)
        dropped_abs = int(drop_abs.sum())
        if dropped_abs:
            df = df[~drop_abs].copy()
        stats["prev_version_exclude_dropped"] = int(stats.get("prev_version_exclude_dropped", 0) or 0) + dropped_abs
        logs.append(f"[prev_version_exclude_abs] nos={len(exclude_prev_nos_set)} ids={len(exclude_prev_ids)} dropped={dropped_abs}")

    # exclude previous version models by No -> model names (supplementary)
    ...
    if excl_names:
        ...
        if name_cols:
            ...
            stats["prev_version_exclude_dropped"] = int(stats.get("prev_version_exclude_dropped", 0) or 0) + dropped
            stats["prev_version_exclude_kept_required"] = 0
            if dropped:
                df = df[~drop_m].copy()
            logs.append(f"[prev_version_exclude] model_names={len(excl_names)} dropped={dropped} kept_required=0")

    # per-rank selection
    # per-rank selection (ENGINE-FIXED):
    # - dedupe key is always (cpu, ram_gb) regardless of YAML.
    # - max_per_product_name is always 1 (within rank) regardless of YAML.
    ...
    # diversity must_cover (ENGINE-FIXED DEFAULT):
    # - If YAML does not specify must_cover, default to cpu_family to enforce chipset diversity.
    must_cover_axes = tuple(policy.diversity.within_rank.must_cover or ())
    if not must_cover_axes:
        must_cover_axes = ("cpu_family",)
        logs.append("[diversity_fixed_default] must_cover=(cpu_family)")
    must_include_if_available = getattr(policy.diversity.within_rank, "must_include_if_available", {}) or {}
    max_per_product_name = 1
    # manufacturer_policy (ENGINE-FIXED):
    # - Always apply soft_dedupe with penalty_weight=0.5 regardless of YAML.
    use_manu, manu_w = (True, 0.5)
    logs.append("[manufacturer_policy_fixed] within_rank=soft_dedupe penalty_weight=0.5")
    logs.append("[dedupe_fixed] key=(cpu,ram_gb) max_per_product_name=1 (GLOBAL across ranks)")

    # product_name cap tracking (GLOBAL across all ranks)
    # - If max_per_product_name=1, once a product_name is selected in any rank,
    #   the same product_name will be blocked in other ranks as well.
    # - Required devices still bypass this cap (but we log if they violate it).
    prod_counts_global: dict[str, int] = {}
    ...
    for rk in RANK_ORDER:
        ...
        has_required_in_rank = False
        if not sub.empty:
            if required_ids and "device_id" in sub.columns:
                has_required_in_rank = bool(sub["device_id"].fillna("").astype(str).str.strip().isin(required_ids).any())
            if (not has_required_in_rank) and required_nos_set and "No" in sub.columns:
                has_required_in_rank = bool(sub["No"].fillna("").astype(str).str.strip().isin(required_nos_set).any())

        if target_n <= 0 and (not has_required_in_rank):
        ...
        req_mask_rank = pd.Series([False] * len(sub_sorted), index=sub_sorted.index)
        if required_ids and "device_id" in sub_sorted.columns:
            req_mask_rank = req_mask_rank | sub_sorted["device_id"].fillna("").astype(str).str.strip().isin(required_ids)
        if required_nos_set and "No" in sub_sorted.columns:
            req_mask_rank = req_mask_rank | sub_sorted["No"].fillna("").astype(str).str.strip().isin(required_nos_set)
        ...
        def _can_take(rr: pd.Series, *, is_required: bool) -> bool:
            ...
            if not pk:
                return True
            return int(prod_counts_global.get(pk, 0)) < max_per_product_name
        ...
        def _mark_taken(rr: pd.Series) -> None:
            ...
            if not pk:
                return
            prod_counts_global[pk] = int(prod_counts_global.get(pk, 0)) + 1

        if bool(req_mask_rank.any()):
            req_df = sub_sorted[req_mask_rank].copy()
            for _, rr in req_df.iterrows():
                did = str(rr.get("device_id") or "").strip()
                no = str(rr.get("No") or "").strip()
                uid = did or (f"NO:{no}" if no else f"IDX:{rr.name}")
                if uid and uid not in used_ids:
                    # required bypasses product_name cap, but log conflicts for transparency
                    if max_per_product_name > 0:
                        pk = _prod_key(rr)
                        if pk and int(prod_counts_global.get(pk, 0)) >= max_per_product_name:
                            logs.append(f"[warn] product_name_global_cap violated by required: rank={rk} product_name={pk}")
                    selected_rows.append(rr)
                    used_ids.add(uid)
                    _mark_taken(rr)
                        ...
                        decision_rows.append(
                            {
                                ...
                                "stage": "must_include",
                                "why_short": (
                                    f"우선 포함 ({'태블릿' if str(ax).strip().lower() == 'form_factor' and str(want).strip().lower() in ('pad','tablet','tab') else str(want).strip()})"
                                    if str(want).strip()
                                    else "우선 포함"
                                ),
        ...
        reps = []
        # (for UI) list alternative candidates within the same profile_core (excluding the chosen one)
        alts_by_core: dict[str, list[str]] = {}

        def _fmt_candidate(rr: pd.Series) -> str:
            n = (str(rr.get("No") or "").strip() or "-")
            name = (
                str(rr.get("product_name") or rr.get("model_name") or "").strip()
                or str(rr.get("device_id") or "").strip()
                or "-"
            )
            return f"{n} / {name}"
        ...
        for pid, g in sub_sorted.groupby("_profile_core", sort=False):
            ...
            if bool(req_mask_rank.any()):
                g2["_is_req"] = pd.Series([False] * len(g2), index=g2.index)
                if required_ids and "device_id" in g2.columns:
                    g2["_is_req"] = g2["_is_req"] | g2["device_id"].fillna("").astype(str).str.strip().isin(required_ids)
                if required_nos_set and "No" in g2.columns:
                    g2["_is_req"] = g2["_is_req"] | g2["No"].fillna("").astype(str).str.strip().isin(required_nos_set)
                g2 = g2.sort_values(by=["_is_req"], ascending=[False])
            # 대표 후보를 reps로:
            # - rotation_nonce==0: 기존처럼 "첫 후보"를 선택(결정론)
            # - rotation_nonce>0: 같은 프로파일 내 후보를 회전시키며 선택(재실행 시 다른 기기 선택 가능)
            picked = None
            if len(g2) > 0:
                if rotation_nonce > 0:
                    # per-profile stable offset so different cores rotate independently
                    seed = f"{inputs.project}|{inputs.version}|{rk}|{str(pid)}"
                    start = (rotation_nonce + _stable_int(seed)) % int(len(g2))
                    # walk circularly to find first candidate satisfying caps
                    for i in range(int(len(g2))):
                        rr = g2.iloc[(start + i) % int(len(g2))]
                    ...
                    for _, rr in g2.iterrows():
                        if _can_take(rr, is_required=False):
                            picked = rr
                            break
            picked2 = picked if picked is not None else g2.iloc[0]
            reps.append(picked2)

            # alternatives (excluding chosen one) for explanation
            chosen_sig = _fmt_candidate(picked2)
            alt_list: list[str] = []
            for _, rr in g2.iterrows():
                s = _fmt_candidate(rr)
                if s == chosen_sig:
        ...
        if not reps_df.empty:
            for _, r in reps_df.iterrows():
                if len(selected_rows) >= target_n and (not bool(req_mask_rank.any())):
                    break
                    ...
                    decision_rows.append(
                        {
                            ...
                            "axes_key": axk,
                            "stage": "core_rep",
                            # show chosen + alternative candidates within the same spec/profile_core
                            "why_short": (
                                (
                                    "대표 1대 (대안 후보: "
                                    + ", ".join((alts_by_core.get(str(core), []) or [])[:3])
                                    + ("..." if len((alts_by_core.get(str(core), []) or [])) > 3 else "")
                                    + ")"
                                )
                                if (alts_by_core.get(str(core), []) or [])
                                else "대표 1대"
                            ),
                            "why": "프로파일 대표(core_rep)로 선정",
                            "why_detail": (
                                f"[core_rep] Rank={rk} 목표={target_n}대 충족을 위해 (cpu,ram) 프로파일 대표로 선정\n"
                                f"- rank_candidates(after_filter/excl)={rank_candidates}, unique_profiles={rank_unique_profiles}\n"
                                ...
                                + f"- dedupe: key={list(dedupe_key)} max_per_product_name={max_per_product_name} allow_axes={list(allow_axes)} allow_duplicates={allow_duplicates} max_dup={max_dup}\n"
                                + f"- tie_breakers: {list(policy.tie_breakers)}"
                            ),
                        }
                    )

        # extras fill (ROUND-ROBIN by profile_core):
        # - fallback_relax(중복 허용 추가)를 없애고, extra 단계에서 목표 수량을 끝까지 채운다.
        # - 같은 스펙(profile_core)에서도 추가로 뽑을 수 있게 하되,
        #   "추가 선정도 1개씩 다른 스펙군으로" 되도록 core별로 1대씩 번갈아(라운드 로빈) 추가한다.
        if len(selected_rows) < target_n:
            sub_sorted2 = sub_sorted.copy()
            # remove already-used by device_id best-effort (used_ids can include NO:* tokens)
            if "device_id" in sub_sorted2.columns and used_ids:
                used_dids = {str(x).strip() for x in used_ids if str(x).strip() and not str(x).startswith("NO:")}
                if used_dids:
                    sub_sorted2 = sub_sorted2[~sub_sorted2["device_id"].fillna("").astype(str).str.strip().isin(used_dids)].copy()

            def _uid(rr: pd.Series) -> str:
                did = str(rr.get("device_id") or "").strip()
                no = str(rr.get("No") or "").strip()
                return did or (f"NO:{no}" if no else f"IDX:{rr.name}")

            # build per-core queues in current order
            core_order: list[str] = []
            core_to_q: dict[str, list[pd.Series]] = {}
            for _, rr in sub_sorted2.iterrows():
                core = str(rr.get("_profile_core") or "")
                if core not in core_to_q:
                    core_to_q[core] = []
                    core_order.append(core)
                core_to_q[core].append(rr)

            def _pop_from_core(core: str) -> Optional[pd.Series]:
                q = core_to_q.get(core) or []
                while q:
                    rr = q.pop(0)
                    uid = _uid(rr)
            ...
            while len(selected_rows) < target_n:
                ...
                for core in core_order:
                    ...
                    # take (we intentionally do NOT enforce max_dup / product_name cap here)
                    selected_rows.append(rr)
                    used_ids.add(uid)
                    _mark_taken(rr)  # keep global accounting for transparency
                    core_counts[core] = core_counts.get(core, 0) + 1
                    axk2 = str(rr.get("_axes_key") or "")
                    core_axes_seen.setdefault(core, set()).add(axk2)

                    # update coverage best-effort (for why_detail)
                    ...
                    decision_rows.append(
                        {
                            ...
                            "why_detail": (
                                f"[extra] Rank={rk} 목표={target_n}대 대비 부족분을 채우기 위해 추가 선정(라운드로빈)\n"
                                f"- rank_candidates(after_filter/excl)={rank_candidates}, unique_profiles={rank_unique_profiles}\n"
                                f"- selection_progress: selected={len(selected_rows)}/{target_n} (including required)\n"
                                f"- profile_core={core}\n"
                                f"- cpu_family={str(rr.get('cpu_family') or '')}\n"
                                ...
                                + ("\n".join(cov_lines2) + "\n" if cov_lines2 else "")
                                + f"- (note) fallback_relax 제거: extra에서 목표 수량까지 채움\n"
                            ),
                        }
                    )
                    progressed = True
        ...
        if not sel_df.empty:
            # mark required by device_id OR No
            is_req = pd.Series([False] * len(sel_df), index=sel_df.index)
            if required_ids and "device_id" in sel_df.columns:
                is_req = is_req | sel_df["device_id"].fillna("").astype(str).str.strip().isin(required_ids)
            if required_nos_set and "No" in sel_df.columns:
                is_req = is_req | sel_df["No"].fillna("").astype(str).str.strip().isin(required_nos_set)
            sel_df["_is_req"] = is_req
    ...
    # FINAL SAFETY: always include required devices in output regardless of rank/filters.
    if required_ids or required_nos_set:
        req_rows_final = df0.head(0).copy()
        if required_ids and "device_id" in df0.columns:
            req_rows_final = df0[df0["device_id"].fillna("").astype(str).str.strip().isin(required_ids)].copy()
        elif required_nos_set and "No" in df0.columns:
            req_rows_final = df0[df0["No"].fillna("").astype(str).str.strip().str.upper().isin(required_nos_set)].copy()

        if not req_rows_final.empty:
            # If a row was already selected, keep it once.
            before = int(len(all_sel))
            all_sel = pd.concat([all_sel, req_rows_final], ignore_index=True, sort=False)
            if "device_id" in all_sel.columns and all_sel["device_id"].fillna("").astype(str).str.strip().ne("").any():
                all_sel = all_sel.drop_duplicates(subset=["device_id"], keep="first")
            elif "No" in all_sel.columns:
                all_sel = all_sel.drop_duplicates(subset=["No"], keep="first")
            added = int(len(all_sel) - before)
            if added > 0:
                logs.append(f"[required_force_include_final] added_to_selected={added}")
                # best-effort: add decision rows so UI shows selection reason
                for _, rr in req_rows_final.iterrows():
                    decision_rows.append(
                        {
                            "rank": str(rr.get("rank") or ""),
                            "device_id": str(rr.get("device_id") or ""),
                            "No": str(rr.get("No") or ""),
                            "product_name": str(rr.get("product_name") or rr.get("model_name") or ""),
                            "brand": str(rr.get("brand") or ""),
                            ...
                            "why_short": "필수 지정",
                            "why": "필수 디바이스로 지정되어 포함",
                            "why_detail": "[필수] 필수 No 입력으로 조건과 무관하게 결과에 포함",
                        }
                    )
    ...
    rules = [
        ...
        f"dedupe: within_rank key={list(dedupe_key)} max_per_product_name={max_per_product_name} allow_axes={list(policy.dedupe.within_rank.allow_duplicates_if_differs_by)} allow_duplicates={policy.dedupe.within_rank.allow_duplicates} max_dup={max_dup}",
