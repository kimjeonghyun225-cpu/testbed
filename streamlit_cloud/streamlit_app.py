...
def _apply_policy_create_ui_overrides(*, base_obj: dict[str, Any], ui_project: str, ui_platforms: list[str]) -> dict[str, Any]:
    """
    ...
    if bool(st.session_state.get("ui_cf_gpu_or_on_create")):
        ...
        # user didn't request exception GPU -> remove
        cf.pop("any_of", None)
                ...
                try:
                    ...
                    try:
                        obj = _apply_policy_create_ui_overrides(
                            base_obj=obj,
                            ui_project=(ui_policy_name or ui_project),
            ...
            if not new_stem:
                ...
                if out_path.exists():
                    ...
                    try:
                        ...
                        # Final: UI 값을 YAML에 강제 반영 (create 화면은 UI가 source of truth)
                        try:
                            pn2 = str(st.session_state.get("ui_policy_name_create") or policy_name or "").strip()
                            obj = _apply_policy_create_ui_overrides(base_obj=obj, ui_project=(pn2 or ui_project), ui_platforms=(ui_platforms if ui_platforms else ["android", "ios"]))
                        except Exception:
                            pass
