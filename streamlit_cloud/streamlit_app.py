...
def _apply_policy_edit_ui_to_obj(*, base_obj: dict[str, Any], ui_project: str) -> dict[str, Any]:
    ...
    if bool(st.session_state.get("ui_cf_gpu_or_on_edit")):
        ...
        if any2:
            cf["any_of"] = [{"include_contains": {"gpu": [any2]}}]
        else:
            cf.pop("any_of", None)
    else:
        cf.pop("any_of", None)
...
<div style="text-align:center; padding-top: 6px;">
  ...
  <div style="margin-top:4px; font-size:13px; color: rgba(17,24,39,0.65);">{subtitle}</div>
    ...
    if screen == "edit":
            ...
            # Prefill when policy changes OR when key fields look empty (prevents "꼬임" after edits/saves)
            need_prefill = st.session_state.get("_policy_edit_ui_last_pick") != pick
            if not need_prefill:
                if not str(st.session_state.get("ui_policy_desc_edit") or "").strip():
                    need_prefill = True
            ...
            with c1:
                ...
                if cbtn3.button("저장(덮어쓰기)", use_container_width=True, type="primary"):
                    try:
                        ...
                        # keep UI values as-is after rerun
    ...
    with c1:
            ...
            # 프로젝트 코드는 UI에서 받지 않고 자동으로 결정한다.
            # - 기본: 현재 선택된 정책(st.session_state["policy_editor_pick"]) 또는 NEWPROJ
            ui_project = str((st.session_state.get("policy_editor_pick") or "NEWPROJ")).strip()
...
h1, h2, h3{
  letter-spacing: -0.02em;
    ...
    if st.sidebar.button("API 설정", use_container_width=True):
        st.session_state["show_api_dialog"] = True
        st.rerun()
    ...
    if st.sidebar.button("정책 설정/편집/생성", use_container_width=True, key="btn_open_policy_catalog"):
        # 항상 목록부터 시작(이전 세션의 edit/view 상태가 남아도 강제 리셋)
  ...
  <div style="color:#111827;line-height:1.7;">
    ...
    try:
        ...
        if req_list_dbg or prev_list_dbg:
            with st.expander("디버그: 필수/직전버전 제외 No 매칭 확인", expanded=False):
                ...
                # pick No-like column from normalized df
                ...
                if not no_col:
                    ...
                    def _match_rows(tokens: list[str]) -> pd.DataFrame:
                        ...
                        m = s_no.isin(want)
                        cols = [c for c in [no_col, did_col, "rank", "platform", "available", "product_name", "model_name"] if c and c in norm_df.columns]
    ...
    with st.spinner("정책 기반 자동 추출 중..."):
        ...
        if len(platforms) == 2:
            ...
            p_android = replace(policy, candidate_filter=replace(cf0, platform="android"))
            p_ios = replace(policy, candidate_filter=replace(cf0, platform="ios"))
            ...
            cf0 = policy.candidate_filter
            p_one = replace(policy, candidate_filter=replace(cf0, platform=platforms[0]))
        ...
        if out_ios is None:
            ...
            try:
                if isinstance(getattr(out_android, "logs", None), list) and out_android.logs:
                    with st.expander("디버그: 엔진 로그(run_policy_v2)", expanded=False):
            ...
            # Debug/UX: show required devices in final output (and pin them to top)
            try:
                ...
                if req_set and "No" in cleaned.columns:
                    ...
                    with st.expander("디버그: 필수 디바이스가 최종 결과에 포함되었는지", expanded=False):
                        st.write({"required_nos": sorted(list(req_set))[:50], "required_in_selected": int(len(req_rows_out)), "selected_total": int(len(cleaned))})
                        if req_rows_out.empty:
                            st.error("필수 No가 최종 결과(selected)에 없습니다. (엔진 최종 강제 포함이 작동했는지 logs 확인 필요)")
                        else:
                            st.dataframe(req_rows_out, use_container_width=True)
                    # pin required rows to top for the main preview
            ...
            try:
                dlog = getattr(out_android, "dedupe_log", None)
                if isinstance(dlog, pd.DataFrame) and (not dlog.empty):
                    with st.expander("디버그: 중복 제거로 제외된 후보(dedupe)", expanded=False):
                        st.caption(
                            "선택 로직에서 (cpu,gpu,ram_gb) 프로파일이 같은 후보는 1개만 남기고 나머지는 제외됩니다. "
                            "아래 목록이 ‘빠진 디바이스’와 일치하면, 공통 원인은 cpu/gpu/ram 중복입니다."
                        )
                        # Attach "kept" representative for the same (rank, profile_core)
                        dlog_view = dlog.copy()
                        try:
                            sel = out_android.selected.copy() if isinstance(getattr(out_android, "selected", None), pd.DataFrame) else pd.DataFrame()
                            if not sel.empty:
                                # normalize rank column name
                                if "rank" not in sel.columns and "Rank" in sel.columns:
                                    sel = sel.rename(columns={"Rank": "rank"})
                                # compute profile_core for selected (engine fixed dedupe_key)
                        ...
                        if "profile_core" in dlog_view.columns:
                            grp = (
                                dlog_view.groupby("profile_core", as_index=False)
                                .size()
            ...
            try:
                ...
                # no_series.str.startswith(...) returns a boolean Series → reduce to a single bool
                no_tmp = bool(no_series.str.startswith("TMP").any())
            except Exception:
                no_empty = True
                no_tmp = False
            ...
            with t1:
                ...
                try:
                    ...
                    if req_set and "No" in cleaned.columns:
                        ...
                        m_req = s_no.isin(req_set)
                        req_rows_out = cleaned[m_req].copy()
                        with st.expander("디버그: 필수 디바이스가 최종 결과에 포함되었는지 (Android)", expanded=False):
                            st.write({"required_nos": sorted(list(req_set))[:50], "required_in_selected": int(len(req_rows_out)), "selected_total": int(len(cleaned))})
                            if req_rows_out.empty:
                                st.error("필수 No가 최종 결과(selected)에 없습니다. (엔진 최종 강제 포함이 작동했는지 logs 확인 필요)")
                            else:
            ...
            with t2:
                st.subheader("Rank별 최종 선택(미리보기)")
                try:
                    if isinstance(getattr(out_ios, "logs", None), list) and out_ios.logs:
                ...
                try:
                    ...
                    if req_set and "No" in cleaned.columns:
                        ...
                        m_req = s_no.isin(req_set)
                        req_rows_out = cleaned[m_req].copy()
                        with st.expander("디버그: 필수 디바이스가 최종 결과에 포함되었는지 (iOS)", expanded=False):
                            st.write({"required_nos": sorted(list(req_set))[:50], "required_in_selected": int(len(req_rows_out)), "selected_total": int(len(cleaned))})
                            if req_rows_out.empty:
                                st.error("필수 No가 최종 결과(selected)에 없습니다. (엔진 최종 강제 포함이 작동했는지 logs 확인 필요)")
                            else:
                ...
                # Debug: show what got dropped by dedupe (same cpu|gpu|ram profile_core)
                try:
                    dlog = getattr(out_ios, "dedupe_log", None)
                    if isinstance(dlog, pd.DataFrame) and (not dlog.empty):
                        with st.expander("디버그: 중복 제거로 제외된 후보(dedupe)", expanded=False):
                            st.caption(
                                "선택 로직에서 (cpu,gpu,ram_gb) 프로파일이 같은 후보는 1개만 남기고 나머지는 제외됩니다. "
                            ...
                            dlog_view = dlog.copy()
                            try:
                                sel = out_ios.selected.copy() if isinstance(getattr(out_ios, "selected", None), pd.DataFrame) else pd.DataFrame()
                                if not sel.empty:
                                    if "rank" not in sel.columns and "Rank" in sel.columns:
                                        sel = sel.rename(columns={"Rank": "rank"})
                                    for k in ["cpu", "gpu", "ram_gb"]:
