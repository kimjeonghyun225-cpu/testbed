...
def _policy_editor_page(*, policy_files: list[Path], selected_policy: Path) -> None:
    ...
    with left:
        ...
        if c4.button("정책 해석", use_container_width=True):
            # Always show deterministic interpretation in a fixed format (no OpenAI dependency).
            ytxt = str(st.session_state.get("policy_yaml_text", ""))
            st.session_state["policy_interpretation"] = _policy_local_summary(
        ...
        if st.session_state.get("policy_interpretation"):
            st.markdown("#### 정책 해석")
            st.code(str(st.session_state.get("policy_interpretation") or ""), language="text")
