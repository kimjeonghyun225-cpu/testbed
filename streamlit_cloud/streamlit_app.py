...
if TYPE_CHECKING:
    # typing only (avoid runtime import/circular deps)
    from app.policy_v2 import PolicyV2
...
def _load_streamlit_secrets_into_env() -> None:
    """
    Streamlit Cloud 운영 정책:
    - API 키/토큰은 코드/파일(local.env 등)에 저장하지 않는다.
    - Streamlit Secrets(st.secrets)에만 저장하고, 앱 시작 시 os.environ으로 주입한다.
    - 이미 환경변수로 주어진 값이 있으면 override 하지 않는다.
    """
    try:
        secrets = st.secrets  # may raise in some environments
    except Exception:
        return
    ...
    for k in keys:
        ...
        try:
            if k in secrets:
                os.environ[k] = _s(secrets.get(k))
        except Exception:
            # secrets 구현/권한 문제 등으로 실패해도 앱은 계속 동작하도록 한다.
            pass


# Cloud 배포본: secrets -> env 주입을 항상 선행
...
def _render_table_auto_width(df: pd.DataFrame, *, max_rows: int = 600, key_prefix: str = "") -> None:
    """
    Streamlit dataframe는 컬럼 폭 자동 맞춤이 제한적이라,
    HTML 테이블로 렌더링해서 "내용에 맞춘 컬럼 폭 + 가로 스크롤"을 제공한다.
    """
    if df is None:
        st.info("표시할 데이터가 없습니다.")
        return
    view = df.head(int(max_rows)).copy()
    # (요청) 표 맨 앞에 1,2,3... 순번 컬럼을 항상 표시
    if "순번" not in view.columns:
        try:
            view.insert(0, "순번", range(1, len(view) + 1))
        except Exception:
            pass
    # (요청) 순번은 좌측 정렬(텍스트)로 보이도록 캐스팅
    if "순번" in view.columns:
        try:
            view["순번"] = view["순번"].astype(str)
        except Exception:
            pass
    # (요청) 컬럼 폭은 Streamlit 기본(auto) 동작에 맡긴다.
    st.dataframe(view, use_container_width=True, height=560, hide_index=True)


def _render_table_fixed_width(df: pd.DataFrame, *, max_rows: int = 600, key_prefix: str = "") -> None:
    """
    고정 컬럼 폭 + 가운데 정렬로 렌더링한다.
    - 필수 디바이스 목록/예외 GPU 매칭 전체처럼 섹션 간 컬럼 폭을 동일 기준으로 맞추고 싶을 때 사용
    """
    if df is None:
        st.info("표시할 데이터가 없습니다.")
        return
...
def _required_only_df(*, master_df: pd.DataFrame, required_nos_text: str) -> pd.DataFrame:
    """
    필수 No 입력이 있을 때, 선정/랭크 목표와 무관하게 '필수 디바이스 목록만' 뽑아 보여주기 위한 DF.
    - 표시 컬럼/순서는 _clean_result_df/_to_user_output_schema 기준을 그대로 따른다.
    """
    toks = _split_csv_list(required_nos_text or "")
    want = {str(x).strip().upper() for x in toks if str(x).strip()}
    if not want or master_df is None or master_df.empty:
        return master_df.head(0).copy() if master_df is not None else pd.DataFrame()

    try:
        from app.policy_v2 import normalize_df_for_policy_v2  # type: ignore
    except Exception:
        normalize_df_for_policy_v2 = None  # type: ignore

    df0 = master_df
    if callable(normalize_df_for_policy_v2):
        try:
            df0 = normalize_df_for_policy_v2(master_df)
        except Exception:
            df0 = master_df

    # map to output schema first (best-effort), then filter by "No"
    view = _dedupe_cols(_clean_result_df(df0))
    if "No" not in view.columns:
        return view.head(0).copy()
    s_no = view["No"].fillna("").astype(str).str.strip().str.upper()
    m = s_no.isin(want)
    out = view[m].copy()
    # (요청) 필수 디바이스 목록은 선정/랭크와 무관하므로 선정 사유를 고정 문구로 표기
    out["선정 사유"] = "필수 지정"
    return out


def _drop_required_rows(df: pd.DataFrame, req_set: set[str]) -> pd.DataFrame:
    if not req_set or df is None or df.empty:
        return df
    if "No" not in df.columns:
        return df
    s_no = df["No"].fillna("").astype(str).str.strip().str.upper()
    mask = s_no.isin(req_set)
    if not mask.any():
        return df
    return df[~mask].copy()


def _count_required_by_rank(
    df: pd.DataFrame, req_set: set[str], *, platform: str | None = None
) -> dict[str, int]:
    if df is None or df.empty or not req_set:
        return {}
    rank_col = "rank" if "rank" in df.columns else ("Rank" if "Rank" in df.columns else "")
    no_col = "No" if "No" in df.columns else ("NO" if "NO" in df.columns else "")
    if not rank_col or not no_col:
        return {}
    s_rank = df[rank_col].fillna("").astype(str).str.strip()
    s_no = df[no_col].fillna("").astype(str).str.strip().str.upper()
    m_req = s_no.isin(req_set)

    if platform:
        plat_col = "platform" if "platform" in df.columns else ("OS" if "OS" in df.columns else "")
        if plat_col:
            s_pl = df[plat_col].fillna("").astype(str).str.strip().str.lower()
            if plat_col == "OS":
                s_pl = s_pl.replace({"aos": "android", "ios": "ios"})
            m_req = m_req & s_pl.eq(str(platform).strip().lower())

    if not bool(m_req.any()):
        return {}
    g = s_rank[m_req].value_counts()
    return {str(k): int(v) for k, v in g.to_dict().items() if str(k).strip()}


def _apply_arch_bits_sidebar_filter(
    df: pd.DataFrame,
    *,
    allow_64: bool,
    allow_32: bool,
    required_nos_set: set[str] | None = None,
) -> tuple[pd.DataFrame, str | None]:
    if df is None or df.empty:
        return df, None
    if (allow_64 and allow_32):
        return df, None
    req_set = {str(x).strip().upper() for x in (required_nos_set or set()) if str(x).strip()}

    plat_col = "platform" if "platform" in df.columns else ("OS" if "OS" in df.columns else "")
    arch_col = "architecture" if "architecture" in df.columns else ("아키텍처" if "아키텍처" in df.columns else "")
    no_col = "No" if "No" in df.columns else ("NO" if "NO" in df.columns else ("no" if "no" in df.columns else ""))
    if not arch_col:
        return df, "아키텍처 컬럼을 찾지 못해(architecture/아키텍처) 64/32bit 필터를 적용하지 않았습니다."

    s_arch = df[arch_col].fillna("").astype(str).str.strip().str.lower()
    m_64 = s_arch.str.contains(r"(64\s*bit|\b64\b)", regex=True)
    m_32 = s_arch.str.contains(r"(32\s*bit|\b32\b)", regex=True)
    m_arch = (m_64 if allow_64 else False) | (m_32 if allow_32 else False)

    req_mask = pd.Series([False] * len(df), index=df.index)
    if req_set and no_col:
        try:
            req_mask = df[no_col].fillna("").astype(str).str.strip().str.upper().isin(req_set)
        except Exception:
            req_mask = pd.Series([False] * len(df), index=df.index)
    ...
    if plat_col:
        ...
        if plat_col == "OS":
            s_pl = s_pl.replace({"aos": "android", "ios": "ios"})
        is_ios = s_pl.eq("ios")
        keep = is_ios | m_arch | req_mask
        if (not allow_64) and (not allow_32):
            return df[keep].copy(), "아키텍처 필터에서 64bit/32bit가 모두 해제되어 일반 디바이스는 제외됩니다. (필수/iOS는 유지)"
        return df[keep].copy(), None
    keep2 = m_arch | req_mask
    if (not allow_64) and (not allow_32):
        return df[keep2].copy(), "아키텍처 필터에서 64bit/32bit가 모두 해제되어 일반 디바이스는 제외됩니다. (필수는 유지)"
    return df[keep2].copy(), None


def _apply_arch_override_to_policy_for_run(policy: object, *, allow_64: bool, allow_32: bool) -> object:
    try:
    ...
    try:
        cf = getattr(policy, "candidate_filter", None)
        if cf is None:
            return policy
        ...
        iv["architecture"] = list(sel)
        cf2 = replace(cf, include_values=iv)
        return replace(policy, candidate_filter=cf2)
    except Exception:
        return policy
...
def _exception_gpu_matches_df(*, master_df: pd.DataFrame, policy: "PolicyV2") -> tuple[pd.DataFrame, list[str]]:
    ...
    if out is not None and not out.empty:
        ...
        # sort by No prefix priority (platform-specific)
        ...
        if no_col:
            ...
            # - Android(AOS): A > AT > R
            # - iOS: IP > IR > R
            p0 = str(getattr(getattr(policy, "candidate_filter", None), "platform", "") or "").strip().lower()
            is_ios = ("ios" in p0) and ("android" not in p0)
...
def _decorate_extra_why(df: pd.DataFrame) -> pd.DataFrame:
    """
    (요청) Rank별 최종 선택에서 '추가 선정' 사유를
    '추가 선정 (선정: No/제품명)' 형태로 구체화한다. (로직 영향 최소: UI/엑셀 출력용 후처리)
    """
    if df is None or df.empty:
        return df
    if "why_short" not in df.columns:
        return df
    out = df.copy()
    m = out["why_short"].fillna("").astype(str).str.strip().eq("추가 선정")
    if not bool(m.any()):
        return out

    def _pick_col(cols: list[str]) -> str | None:
        for c in cols:
            if c in out.columns:
                return c
        return None

    no_col = _pick_col(["No", "NO", "no"])
    pn_col = _pick_col(["제품명", "product_name", "model_name"])
    s_no = out[no_col].fillna("").astype(str).str.strip() if no_col else ""
    s_pn = out[pn_col].fillna("").astype(str).str.strip() if pn_col else ""

    if no_col and pn_col:
        out.loc[m, "why_short"] = (s_no[m] + "/" + s_pn[m]).apply(lambda x: f"추가 선정 (선정: {x})")
    elif no_col:
        out.loc[m, "why_short"] = s_no[m].apply(lambda x: f"추가 선정 (선정: {x})")
    elif pn_col:
        out.loc[m, "why_short"] = s_pn[m].apply(lambda x: f"추가 선정 (선정: {x})")
    else:
        out.loc[m, "why_short"] = "추가 선정"
    return out
...
def _policy_local_summary_simple(yaml_text: str) -> str:
    ...
    max_per_product_name = dw.get("max_per_product_name") if isinstance(dw, dict) else None
    allow_dupes_if_diff = _as_str_list(dw.get("allow_duplicates_if_differs_by") if isinstance(dw, dict) else "")
    # NOTE: 엔진(v2)의 dedupe 기준은 정책(YAML)과 무관하게 고정됨.
    # - 현재 엔진 고정키: cpu + ram_gb (GPU는 dedupe에서 무시)
    # 요약/해석 화면도 엔진 기준과 일치하도록 표시한다.
    dedupe_key = ["cpu", "ram_gb"]
    ...
    if dedupe_key:
        if set(dedupe_key) >= {"cpu", "ram_gb"}:
            dedupe_sentence_parts.append("같은 스펙(CPU+RAM)은 대표 1대만")
        else:
...
<div style="text-align:center; padding-top: 6px;">
  ...
  <div style="margin-top:4px; font-size:13px; color: rgba(17,24,39,0.65);">{subtitle}</div>
    ...
    st.session_state.setdefault("policy_edit_yaml_text_touched", False)
    if st.session_state.get("_policy_edit_last_pick") != pick:
        ...
        st.session_state["policy_edit_yaml_text_touched"] = False
    ...
    if screen == "edit":
            ...
            # base policy obj:
            # - UI 편집 모드에서는 항상 "선택된 정책 파일(raw_text)" 기준으로 미리보기 YAML을 생성한다.
            # - 직접 YAML 편집을 실제로 했다면(touched), 그 YAML을 기준으로 미리보기를 만들 수 있다.
            try:
                use_direct_base = bool(st.session_state.get("policy_edit_yaml_text_touched"))
                direct_yaml = str(st.session_state.get("policy_edit_yaml_text") or "").strip()
                # If direct YAML is "touched" but empty, fall back to raw_text so we don't lose the base policy.
                if use_direct_base and (not direct_yaml):
                    use_direct_base = False
                base_yaml = direct_yaml if use_direct_base else str(raw_text or "")
                base_obj2 = yaml.safe_load(base_yaml) or {}
            ...
            # If missing, fall back to policy filename stem to avoid showing NEWPROJ unexpectedly.
            ui_project_for_edit = (
                str(base_obj2.get("project") or "").strip()
                or str(policy_path.stem).strip()
                or "NEWPROJ"
            )
            ...
            with c1:
                ...
                if cbtn1.button("YAML 검증", use_container_width=True):
                    try:
                        ...
                        use_direct = bool(st.session_state.get("policy_edit_yaml_text_touched"))
                        direct_yaml = str(st.session_state.get("policy_edit_yaml_text") or "").strip()
                        preview_yaml = str(st.session_state.get("policy_edit_yaml_preview") or merged_yaml).strip()
                        src_yaml = direct_yaml if (use_direct and direct_yaml) else preview_yaml
                        tmp_path.write_text(src_yaml, encoding="utf-8")
                ...
                if cbtn3.button("저장(덮어쓰기)", use_container_width=True, type="primary"):
                    try:
                        ...
                        try:
                            ...
                            preview_yaml = str(st.session_state.get("policy_edit_yaml_preview") or merged_yaml).strip()
                            src_yaml = direct_yaml if (use_direct and direct_yaml) else preview_yaml
                            obj_save = yaml.safe_load(src_yaml) or {}
            ...
            with c2:
                ...
                with st.expander("고급: YAML 직접 편집(선택)", expanded=False):
                    st.caption("필요한 경우에만 직접 편집하고 저장(덮어쓰기) 버튼을 클릭하세요.")

                    def _mark_policy_edit_yaml_text_touched() -> None:
                        st.session_state["policy_edit_yaml_text_touched"] = True

                    st.text_area(
                        "YAML(직접 편집)",
                        key="policy_edit_yaml_text",
                        height=420,
                        on_change=_mark_policy_edit_yaml_text_touched,
                    )
    ...
    with c1:
            ...
            if st.button("요구사항 문장 자동 생성", use_container_width=True, key="btn_fill_natural_create"):
                ...
                if not _safe_str(api_key):
                    st.error("OpenAI API 키가 필요합니다. Streamlit Cloud 앱 설정의 Secrets(또는 환경변수)에 OPENAI_API_KEY를 등록하세요.")
                    return
                try:
                    ...
                    usr_txt = (
                        ...
                        "- (필수) dedupe.within_rank.key는 반드시 [cpu, ram_gb] 로 넣어라\n"
                    ...
                    msg = str(e).strip()
                    st.error("YAML 초안 생성에 실패했습니다.")
                    if msg:
                        st.code(msg[:800])
                    st.caption("확인: OPENAI_API_KEY / OPENAI_MODEL Secrets/환경변수 값, 모델명 유효성(오타/권한)을 확인하세요.")
                    return
            ...
            with st.expander("디버그: 예시 10개 OpenAI YAML 생성/검증(버튼 1번)", expanded=False):
                ...
                if st.button("예시 10개 실행(생성+검증)", use_container_width=True, key="btn_run_10_examples"):
                    api_key = os.getenv("OPENAI_API_KEY") or ""
                    if not _safe_str(api_key):
                        st.error("OpenAI API 키가 필요합니다. Streamlit Cloud Secrets/환경변수에 OPENAI_API_KEY를 설정하세요.")
                        return
...
/* Main area background: avoid white strip between sidebar and content when padding is used */
...
/* Sidebar: always expanded (hide collapse/expand controls) */
/* Newer/older Streamlit variants use different testids */
...
/* Sidebar spacing: reduce blank areas */
...
/* Reduce divider/heading margins inside sidebar to avoid large blank gaps */
...
/* Ensure inputs/selects fit inside sidebar width */
...
/* Main content width: make result tables much wider */
...
/* Auto-width HTML tables (result preview) */
.auto_tbl_wrap{
  overflow-x: auto;
  overflow-y: auto;
  max-height: 560px;
...
.auto_tbl th, .auto_tbl td{
  white-space: nowrap;
  border-bottom: 1px solid rgba(0,0,0,0.06);
  padding: 6px 10px;
  font-size: 12px;
  /* (요청) Rank별 최종 선택(미리보기) 포함, 결과값/헤더 전체 중앙 정렬 */
  text-align: center;
...
/* Fixed-width tables (only for required / exception sections) */
.auto_tbl_fixed{
  width: 100%;
  min-width: 100%;
  table-layout: fixed;
}
.auto_tbl_fixed th, .auto_tbl_fixed td{
  text-align: center;
  vertical-align: middle;
  overflow: hidden;
...
/* Column widths (18 cols: 순번 + 17 cols with '아키텍처') - only when fixed */
.auto_tbl_fixed.auto_tbl_with_arch{
  /* Fill container width, but keep a sane minimum width */
  width: 100%;
  min-width: 2000px;
}
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(1), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(1){ width: 56px; }   /* 순번 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(2), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(2){ width: 60px; }   /* Rank */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(3), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(3){ width: 90px; }   /* No */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(4), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(4){ width: 70px; }   /* 상태 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(5), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(5){ width: 60px; }   /* OS */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(6), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(6){ width: 200px; }  /* 제품명 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(7), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(7){ width: 140px; }  /* 모델번호 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(8), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(8){ width: 110px; }  /* 제조사 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(9), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(9){ width: 110px; }  /* 디바이스 타입 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(10), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(10){ width: 110px; }  /* 타겟 국가 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(11), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(11){ width: 190px; }/* CPU */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(12), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(12){ width: 170px; }/* GPU */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(13), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(13){ width: 80px; } /* RAM */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(14), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(14){ width: 130px; }/* DISPLAY */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(15), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(15){ width: 90px; } /* 출시 년도 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(16), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(16){ width: 90px; } /* 아키텍처 */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(17), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(17){ width: 80px; } /* OS Ver */
/* 선정 사유(마지막 컬럼)는 오른쪽 끝까지 확장되도록 width 고정하지 않음 (최소폭만 보장) */
.auto_tbl_fixed.auto_tbl_with_arch th:nth-child(18), .auto_tbl_fixed.auto_tbl_with_arch td:nth-child(18){ min-width: 220px; }

/* Column widths (17 cols: 순번 + 16 cols without '아키텍처' - iOS) - only when fixed */
.auto_tbl_fixed.auto_tbl_no_arch{
  /* Fill container width, but keep a sane minimum width */
  width: 100%;
  min-width: 1910px;
}
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(1), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(1){ width: 56px; }    /* 순번 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(2), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(2){ width: 60px; }    /* Rank */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(3), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(3){ width: 90px; }    /* No */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(4), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(4){ width: 70px; }    /* 상태 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(5), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(5){ width: 60px; }    /* OS */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(6), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(6){ width: 200px; }   /* 제품명 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(7), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(7){ width: 140px; }   /* 모델번호 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(8), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(8){ width: 110px; }   /* 제조사 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(9), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(9){ width: 110px; }   /* 디바이스 타입 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(10), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(10){ width: 110px; }   /* 타겟 국가 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(11), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(11){ width: 190px; } /* CPU */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(12), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(12){ width: 170px; } /* GPU */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(13), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(13){ width: 80px; }  /* RAM */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(14), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(14){ width: 130px; } /* DISPLAY */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(15), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(15){ width: 90px; }  /* 출시 년도 */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(16), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(16){ width: 80px; }  /* OS Ver */
/* 선정 사유(마지막 컬럼)는 오른쪽 끝까지 확장되도록 width 고정하지 않음 (최소폭만 보장) */
.auto_tbl_fixed.auto_tbl_no_arch th:nth-child(17), .auto_tbl_fixed.auto_tbl_no_arch td:nth-child(17){ min-width: 220px; }
/* Sidebar */
section[data-testid="stSidebar"]{
...
h1, h2, h3{
  letter-spacing: -0.02em;
    ...
    top_download_slot = None
    if st.session_state.get("page_mode") != "policy":
        st.title("테스트 디바이스 자동 추출 시스템")
        # (요청) 엑셀 다운로드 버튼은 타이틀 바로 아래에 표시(실행 후에만 채워짐)
    ...
    st.sidebar.subheader("프로젝트 정책 선택")
    ...
    if st.sidebar.button("정책 설정/편집/생성", use_container_width=True, key="btn_open_policy_catalog"):
        ...
        # (API 설정 UI 제거) 과거 상태키 호환을 위해 남겨둔 키는 안전하게 무시
        st.session_state["policy_screen"] = "catalog"
    ...
    # (레이아웃) 업로드와 정책 섹션 사이 간격을 줄이기 위해 API 경고는 아래로 내린다.
    # API 키/토큰은 Secrets/환경변수에서 자동 주입되며, UI 입력/저장은 제공하지 않는다.
    if not _safe_str(os.getenv("OPENAI_API_KEY")):
        st.sidebar.warning("OPENAI_API_KEY 미설정 (Streamlit Cloud Secrets/환경변수에서 설정 필요)")
    st.sidebar.divider()
    ...
    st.sidebar.subheader("아키텍처 필터")
    acol1, acol2 = st.sidebar.columns(2)
    with acol1:
        st.checkbox("64bit", value=bool(st.session_state.get("run_arch_64", True)), key="run_arch_64")
    with acol2:
        st.checkbox("32bit", value=bool(st.session_state.get("run_arch_32", True)), key="run_arch_32")
    ...
    required_nos = st.sidebar.text_input(
        ...
        key="required_nos",
        placeholder="해당 디바이스 강제 포함 예:AP123,P001",
        help="선택 사항 - 지정 시 해당 No는 이번 결과에 강제 포함됩니다. (아키텍처 필터와 무관)",
    )
    prev_excl_nos = st.sidebar.text_input(
        "직전 버전 제외 No(콤마, 선택)",
        value=st.session_state.get("prev_excl_nos", ""),
        key="prev_excl_nos",
        placeholder="해당 디바이스 강제 제외 예:AP001,IP001",
        help="해당 No들의 제품명/모델명은 이번 버전에서 제외됩니다.",
    )

    st.sidebar.divider()
    # 같은 스펙(대표 1대 선택) 로테이션:
    # - 재실행 시 같은 스펙 그룹에서 다른 기기가 대표로 뽑히도록 "회전 nonce"를 증가시킨다.
    st.sidebar.caption("같은 스펙 로테이션(선택)")
    rotate_same_spec = st.sidebar.checkbox(
        "재실행 시 같은 스펙에서 다른 기기 뽑기",
        value=True,
        help="같은 cpu+ram 스펙 그룹에서 대표 1대를 고를 때, 실행할 때마다 후보를 회전시켜 다른 기기가 선택될 수 있습니다.",
        key="rotate_same_spec",
    )
    run_btn = st.sidebar.button("자동 추출 실행", type="primary", disabled=(upload is None))
    # rotation nonce: click마다 증가 (옵션 OFF면 0 고정)
    st.session_state.setdefault("rotation_nonce", 0)
    if run_btn:
        if rotate_same_spec:
            st.session_state["rotation_nonce"] = int(st.session_state.get("rotation_nonce") or 0) + 1
        else:
            st.session_state["rotation_nonce"] = 0

    # 어디서든(정책 편집/생성/상세 포함) "자동 추출 실행"이 동작하도록:
    # - policy 화면에서 눌렀다면 run 화면으로 전환하고, 다음 rerun에서 실행을 이어간다.
    st.session_state.setdefault("_pending_run", False)
    if run_btn and st.session_state.get("page_mode") == "policy":
        st.session_state["_pending_run"] = True
        st.session_state["page_mode"] = "run"
        # query param 복원 로직에 의해 policy_screen이 다시 붙는 혼동을 줄이기 위해 초기화
        st.session_state["policy_screen"] = "catalog"
        try:
            qp = getattr(st, "query_params", None)
            if qp is not None:
    ...
    run_requested = bool(run_btn) or bool(st.session_state.get("_pending_run"))
    if run_requested and bool(st.session_state.get("_pending_run")) and (not run_btn):
        # pending으로 이어진 실행은 1회만 처리
        st.session_state["_pending_run"] = False
    ...
    # (요청) 실행 전 메인 화면에서는 "결과" 문구를 노출하지 않음
    result_box = st.empty()
    ...
    if not run_requested:
        with result_box.container():
            st.markdown(
                """
<div style="background:#ffffff;border:1px solid rgba(0,0,0,0.06);border-radius:16px;padding:28px;text-align:left;">
  <div style="font-size:20px;font-weight:800;margin-bottom:10px;color:#111827;">빠른 시작 가이드</div>
  <div style="color:#111827;line-height:1.75;max-width:920px;">
    <div style="margin-bottom:10px;">1) 좌측 상단에서 마스터 디바이스 목록(Excel/CSV)을 업로드하세요.</div>
    <div style="margin-bottom:10px;">2) 프로젝트 정책을 선택하세요.</div>
    <div style="margin-bottom:10px;">3) 플랫폼(Android/iOS)과 아키텍처(64bit/32bit)를 선택하세요.</div>
    <div style="margin-bottom:10px;">4) Rank별 목표 수량을 입력하세요. (A+, A, B, C, D, 무등급)</div>
    <div style="margin-bottom:10px;">5) (선택) 타겟 국가/NOTE 제외, 필수 포함, 직전 버전 제외 조건을 입력하세요.</div>
    <div style="margin-bottom:10px;">6) 사이드바 하단의 자동 추출 실행을 클릭하세요.</div>
    <div>7) 실행 후 상단 결과 다운로드 버튼에서 엑셀을 받거나, 아래 표에서 결과를 확인하세요.</div>
  </div>
</div>
                """,
                unsafe_allow_html=True,
            ...
            # (요청) 정책 생성/수정 가이드는 빠른 시작 가이드와 동일한 "하얀 박스" 템플릿으로 표시
            st.markdown(
                """
<div style="margin-top:14px;background:#ffffff;border:1px solid rgba(0,0,0,0.06);border-radius:16px;padding:28px;text-align:left;">
<div style="font-size:20px;font-weight:800;margin-bottom:10px;color:#111827;">정책 생성 및 수정 가이드</div>
<div style="color:#111827;line-height:1.75;max-width:920px;">
  <div style="margin-bottom:10px;"><b>방법 1: UI 입력 기반</b></div>
  <div style="margin-bottom:10px;">화면 좌측에서 조건을 직접 입력하는 방식입니다.</div>
  <div style="margin-bottom:10px;">1) 프로젝트/정책 이름을 입력하세요. (필수)</div>
  <div style="margin-bottom:10px;">2) 설명을 입력하세요.</div>
  <div style="margin-bottom:10px;">3) (선택) 제조사 필터를 입력하세요. 예: SAMSUNG, APPLE</div>
  <div style="margin-bottom:10px;">4) 아키텍처를 선택하세요. (64bit / 32bit 체크)</div>
  <div style="margin-bottom:10px;">5) (선택) GPU 포함 키워드를 입력하세요. 예: powervr / adreno</div>
  <div style="margin-bottom:10px;">6) (선택) RAM 범위, OS 최소 체크 후 값을 입력하세요.</div>
  <div style="margin-bottom:10px;">7) [요구사항 문장 자동 생성] 버튼을 클릭하세요.</div>
  <div style="margin-bottom:10px;">8) 우측에 YAML이 자동 생성됩니다.</div>
  <div style="margin-bottom:10px;">9) 파일명 입력 후 [새 파일로 저장]을 클릭하세요.</div>
  <div style="margin-bottom:10px;">10) 추출 화면으로 자동 이동됩니다.</div>
  <div style="margin-bottom:10px;"></div>

  <div style="margin-bottom:10px;"><b>방법 2: LLM 프롬프트 기반</b></div>
  <div style="margin-bottom:10px;">자연어로 조건을 입력하면 AI가 YAML을 만들어주는 방식입니다.</div>
  <div style="margin-bottom:10px;">1) 프로젝트 정책 이름을 입력하세요. (필수)</div>
  <div style="margin-bottom:10px;">2) 화면 중간의 “정책 프롬프트 확인/입력” 영역에 요구사항을 작성하세요.</div>
  <div style="margin-bottom:10px;">3) [YAML 초안 생성(OpenAI)] 버튼을 클릭하세요.</div>
  <div style="margin-bottom:10px;">4) 우측에 YAML이 자동 생성됩니다.</div>
  <div style="margin-bottom:10px;">5) 파일명 입력 후 [새 파일로 저장]을 클릭하세요.</div>
  <div style="margin-bottom:10px;">6) 추출 화면으로 자동 이동됩니다.</div>
  <div style="margin-bottom:10px;"></div>

  <div style="margin-bottom:10px;">프롬프트 예시</div>
  <div style="margin-bottom:10px;">- Android만 + 삼성만 + GPU에 PowerVR 모두 포함 + RAM 3~8GB</div>
  <div style="margin-bottom:10px;">- iOS만 + OS 15 이상</div>
  <div style="margin-bottom:10px;">- 64bit만 + OS 9 이상</div>
  <div style="margin-bottom:10px;"></div>

  <div style="margin-bottom:10px;"><b>정책 수정 방법</b></div>
  <div style="margin-bottom:10px;">기존 정책을 불러와서 일부만 바꾸는 방식입니다.</div>
  <div style="margin-bottom:10px;">1) “정책 수정하기” 화면에 진입하세요.</div>
  <div style="margin-bottom:10px;">2) 수정할 정책을 선택하세요. → 기존 값이 자동 로드됩니다.</div>
  <div style="margin-bottom:10px;">3) 좌측 UI에서 원하는 항목을 수정하세요.</div>
  <div style="margin-bottom:10px;">4) [저장(덮어쓰기)]를 클릭하세요.</div>
  <div>5) 추출 화면으로 자동 이동됩니다.</div>
</div>
</div>
                """,
                unsafe_allow_html=True,
    ...
    # (요청) YAML 정책은 그대로 두고, 이번 실행에서만 아키텍처(32/64) 조건을 임시 적용
    policy_run = _apply_arch_override_to_policy_for_run(
        policy,
        allow_64=bool(st.session_state.get("run_arch_64", True)),
        allow_32=bool(st.session_state.get("run_arch_32", True)),
    )

    req_list_input = _split_csv_list(str(required_nos or ""))
    req_required_set = {str(x).strip().upper() for x in req_list_input if str(x).strip()}

    # (요청) 사이드바 64bit/32bit 체크박스에 따라 출력 대상을 제한
    norm_df, arch_warn = _apply_arch_bits_sidebar_filter(
        norm_df,
        allow_64=bool(st.session_state.get("run_arch_64", True)),
        allow_32=bool(st.session_state.get("run_arch_32", True)),
        required_nos_set=req_required_set,
    )
    if arch_warn:
        st.sidebar.warning(str(arch_warn))

    # (요청) 필수 디바이스를 Rank 표에서 숨길 경우에도 Rank 목표 수량을 채우기 위해,
    # 엔진 실행의 rank_targets를 "필수 개수만큼" 보정하여 추가로 뽑는다.
    req_counts_android = _count_required_by_rank(norm_df, req_required_set, platform="android")
    req_counts_ios = _count_required_by_rank(norm_df, req_required_set, platform="ios")

    def _add_req_counts(base: dict[str, int], add: dict[str, int]) -> dict[str, int]:
        out = {}
        for k, v in (base or {}).items():
            kk = str(k)
            out[kk] = int(v or 0) + int(add.get(kk, 0) or 0)
        return out

    rank_targets_android_adj = _add_req_counts(rank_targets_android, req_counts_android)
    rank_targets_ios_adj = _add_req_counts(rank_targets_ios, req_counts_ios)

    inputs = PolicyV2RunInputs(
        project=policy_run.project,
        version=version,
        rank_targets=(rank_targets_android_adj if (len(platforms) == 1 and platforms[0] == "android") else rank_targets_ios_adj),
        ...
        rotation_nonce=int(st.session_state.get("rotation_nonce") or 0),
    )
    ...
    with st.spinner("정책 기반 자동 추출 중..."):
        ...
        if len(platforms) == 2:
            ...
            cf0 = policy_run.candidate_filter
            p_android = replace(policy_run, candidate_filter=_candidate_filter_for_platform(cf0, "android"))
            p_ios = replace(policy_run, candidate_filter=_candidate_filter_for_platform(cf0, "ios"))

            out_android = run_policy_v2(master_df=norm_df, policy=p_android, inputs=replace(inputs, rank_targets=rank_targets_android_adj))
            out_ios = run_policy_v2(master_df=norm_df, policy=p_ios, inputs=replace(inputs, rank_targets=rank_targets_ios_adj))
        else:
            from dataclasses import replace

            cf0 = policy_run.candidate_filter
            p_one = replace(policy_run, candidate_filter=_candidate_filter_for_platform(cf0, platforms[0]))
    ...
    with result_box.container():
        # (요청) 필수 디바이스/예외 목록은 멀티플랫폼(탭) UI에서는 iOS 탭 옆의 별도 탭으로 제공한다.
        req_tokens = _split_csv_list(str(required_nos or ""))
        req_only = _required_only_df(master_df=norm_df, required_nos_text=str(required_nos or ""))

        if out_ios is None:
            # single-platform: keep required list at top
            if req_only is not None and not req_only.empty:
                st.subheader("필수 디바이스 목록")
                st.caption("필수 No 입력값에 매칭되는 디바이스만 표시합니다. (랭크 목표/선정 로직과 무관)")
                _render_table_fixed_width(req_only, max_rows=600, key_prefix="tab_req_only")
            elif req_tokens:
                st.subheader("필수 디바이스 목록")
                st.caption("필수 No 입력값에 매칭되는 디바이스만 표시합니다. (랭크 목표/선정 로직과 무관)")
                st.warning("필수 No가 업로드 데이터에서 0건 매칭되었습니다. (No 표기/공백/숨은 문자 여부를 확인해주세요)")

            st.subheader("Rank별 최종 선택(미리보기)")
            sel_with_why = _decorate_extra_why(_attach_why(out_android.selected, out_android.decision_log))
            cleaned = _dedupe_cols(_clean_result_df(sel_with_why))

            cleaned = _drop_required_rows(cleaned, req_required_set)
            _render_table_auto_width(cleaned, max_rows=600, key_prefix="tab_rank_single")

            # 예외 GPU 전체 목록(선정과 별개)
            if isinstance(exc_df_a, pd.DataFrame) and not exc_df_a.empty:
                st.subheader("예외 GPU 매칭 전체")
                st.caption(
                ...
                # 예외 GPU 매칭 전체도 사용자 요청으로 '선정 사유'를 표기한다(선정과 별개인 조건 매칭임을 명시)
                exc_view["선정 사유"] = "예외 GPU 조건 매칭(선정과 별개)"
                _render_table_fixed_width(exc_view, max_rows=600, key_prefix="tab_exc_single_aos")
        else:
            show_req_tab = bool(req_only is not None and not req_only.empty)
            show_exc_tab = bool((isinstance(exc_df_a, pd.DataFrame) and not exc_df_a.empty) or (isinstance(exc_df_i, pd.DataFrame) and not exc_df_i.empty))
            if (not show_req_tab) and req_tokens:
                st.warning("필수 No가 업로드 데이터에서 0건 매칭되었습니다. (No 표기/공백/숨은 문자 여부를 확인해주세요)")

            tab_labels = ["Android(AOS) 결과", "iOS 결과"]
            if show_req_tab:
                tab_labels.append("필수 디바이스 목록")
            if show_exc_tab:
                tab_labels.append("예외 결과목록")

            tabs = st.tabs(tab_labels)
            t1 = tabs[0]
            t2 = tabs[1]
            idx = 2
            t_req = tabs[idx] if show_req_tab else None
            idx = idx + 1 if show_req_tab else idx
            t_exc = tabs[idx] if show_exc_tab else None
            with t1:
                st.subheader("Rank별 최종 선택(미리보기)")
                sel_with_why = _decorate_extra_why(_attach_why(out_android.selected, out_android.decision_log))
                cleaned = _dedupe_cols(_postprocess_output_for_platform(_clean_result_df(sel_with_why), "android"))
                cleaned = _drop_required_rows(cleaned, req_required_set)
                _render_table_auto_width(cleaned, max_rows=600, key_prefix="tab_rank_aos")
            with t2:
                st.subheader("Rank별 최종 선택(미리보기)")
                sel_with_why = _decorate_extra_why(_attach_why(out_ios.selected, out_ios.decision_log))
                cleaned = _dedupe_cols(_postprocess_output_for_platform(_clean_result_df(sel_with_why), "ios"))
                cleaned = _drop_required_rows(cleaned, req_required_set)
                _render_table_auto_width(cleaned, max_rows=600, key_prefix="tab_rank_ios")
            if show_req_tab and t_req is not None:
                with t_req:
                    st.subheader("필수 디바이스 목록")
                    st.caption("필수 No 입력값에 매칭되는 디바이스만 표시합니다. (랭크 목표/선정 로직과 무관)")
                    _render_table_fixed_width(req_only, max_rows=600, key_prefix="tab_req_only")
            if show_exc_tab and t_exc is not None:
                with t_exc:
                    st.subheader("예외 GPU 매칭 전체")
                    st.caption("이 목록은 dedupe/랭크 목표와 무관한 ‘매칭 전체’입니다.")
                    sub_labels = []
                    if isinstance(exc_df_a, pd.DataFrame) and not exc_df_a.empty:
                        sub_labels.append("Android(AOS) 예외")
                    if isinstance(exc_df_i, pd.DataFrame) and not exc_df_i.empty:
                        sub_labels.append("iOS 예외")
                    if not sub_labels:
                        st.info("예외 GPU 매칭 결과가 없습니다.")
                    elif len(sub_labels) == 1:
                        if "Android" in sub_labels[0]:
                            st.caption(f"예외 GPU(AOS): {', '.join(exc_toks_a) if exc_toks_a else '-'} / 총 {len(exc_df_a)}대")
                            exc_view = _dedupe_cols(_postprocess_output_for_platform(_clean_result_df(exc_df_a), "android"))
                            exc_view["선정 사유"] = "예외 GPU 조건 매칭(선정과 별개)"
                            _render_table_fixed_width(exc_view, max_rows=600, key_prefix="tab_exc_aos")
                        else:
                            st.caption(f"예외 GPU(iOS): {', '.join(exc_toks_i) if exc_toks_i else '-'} / 총 {len(exc_df_i)}대")
                            exc_view = _dedupe_cols(_postprocess_output_for_platform(_clean_result_df(exc_df_i), "ios"))
                            exc_view["선정 사유"] = "예외 GPU 조건 매칭(선정과 별개)"
                            _render_table_fixed_width(exc_view, max_rows=600, key_prefix="tab_exc_ios")
                    else:
                        ex_a, ex_i = st.tabs(sub_labels)
                        with ex_a:
                            st.caption(f"예외 GPU(AOS): {', '.join(exc_toks_a) if exc_toks_a else '-'} / 총 {len(exc_df_a)}대")
                            exc_view = _dedupe_cols(_postprocess_output_for_platform(_clean_result_df(exc_df_a), "android"))
                            exc_view["선정 사유"] = "예외 GPU 조건 매칭(선정과 별개)"
    ...
    with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
        if out_ios is None:
            ...
            for rk in ["A+", "A", "B", "C", "D", "-"]:
                ...
                df_rk2 = _decorate_extra_why(_attach_why(df_rk, out_android.decision_log))
                df_rk2 = _drop_required_rows(df_rk2, req_required_set)
            ...
            all2 = _decorate_extra_why(_attach_why(out_android.selected, out_android.decision_log))
            all2 = _drop_required_rows(all2, req_required_set)
            ...
            for rk in ["A+", "A", "B", "C", "D", "-"]:
                ...
                df_rk2 = _decorate_extra_why(_attach_why(df_rk, out_ios.decision_log))
                df_rk2 = _drop_required_rows(df_rk2, req_required_set)
                _dedupe_cols(_postprocess_output_for_platform(_clean_result_df(df_rk2), "ios")).to_excel(writer, sheet_name=name[:31], index=False)
            all_a = _decorate_extra_why(_attach_why(out_android.selected, out_android.decision_log))
            all_a = _drop_required_rows(all_a, req_required_set)
            all_i = _decorate_extra_why(_attach_why(out_ios.selected, out_ios.decision_log))
            all_i = _drop_required_rows(all_i, req_required_set)
    ...
    dl_policy_name = _safe_policy_filename(str(policy_path.stem)) or str(policy_path.stem)
    if top_download_slot is not None:
        top_download_slot.download_button(
            "엑셀 다운로드(멀티시트)",
            data=buf.getvalue(),
            file_name=f"{dl_policy_name}_{version}_selection.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )

    with download_box.container():
        # (요청) 디버그는 상위 1개 expander로 감싸서 기본 숨김
        with st.expander("디버그(필요 시)", expanded=False):
            ...
            try:
                if req_required_set:
                    with st.expander("디버그: 필수 디바이스가 최종 결과에 포함되었는지", expanded=False):
                        st.write({"required_nos": sorted(list(req_required_set))[:80]})
                        # Android
                        try:
                            ...
                            if "No" in clean_a.columns:
                                ...
                                req_rows_a = clean_a[s_no_a.isin(req_required_set)].copy()
                        ...
                        try:
                            if out_ios is not None:
                                ...
                                if "No" in clean_i.columns:
                                    ...
                                    req_rows_i = clean_i[s_no_i.isin(req_required_set)].copy()
            ...
            # Debug: show what got dropped by dedupe (same cpu|ram profile_core)
            def _render_dedupe_debug(out_obj: Any, *, title: str) -> None:
                try:
                    ...
                    with st.expander(title, expanded=False):
                        st.caption(
                            "선택 로직에서 (cpu,ram_gb) 프로파일이 같은 후보는 1개만 남기고 나머지는 제외됩니다. "
                            "아래 목록이 ‘빠진 디바이스’와 일치하면, 공통 원인은 cpu/ram 중복입니다."
                        )
                        dlog_view = dlog.copy()
                        try:
                            ...
                            if not sel.empty:
                                ...
                                for k in ["cpu", "ram_gb"]:
                                    if k not in sel.columns:
                                        sel[k] = ""
                                parts = [sel[k].fillna("").astype(str).str.strip() for k in ["cpu", "ram_gb"]]
                                prof = parts[0]
                                for p in parts[1:]:
                                    prof = prof + "|" + p
                        ...
                        st.write(
                            {
                                ...
                                "unique_profile_core": int(len(set(dlog_view["profile_core"]))) if "profile_core" in dlog_view.columns else None,
